...
namespace IAR.Domain.InformationAssets
{
    public class Asset : IValidate
    {
        ...
        /// </summary>
        public virtual ICollection<AssetLawfulProcessingCondition> LawfulProcessingConditions { get; set; }

        /// <summary>
        /// Gets or sets a collection of <see cref="PersonalDataSource"/> containing Asset locations where the asset is held.
        /// </summary>
        public virtual ICollection<AssetPersonalDataSource> PersonalDataSources { get; set; }
        ...
        /// </summary>
        public virtual ICollection<AssetPersonalDataSpecialCategory> PersonalDataSpecialCategories { get; set; }
        ...
        /// </summary>
        public virtual ICollection<AssetSpecialCategoryLawfulProcessingCondition> SpecialCategoryLawfulProcessingConditions { get; set; }
        ...
        /// </summary>
        public virtual ICollection<AssetApplicableRight> ApplicableRights { get; set; }

        /// <summary>
        /// Gets or sets a collection of <see cref="OrganisationalSecurityMeasure"/> containing Asset locations where the asset is held.
        /// </summary>
        public virtual ICollection<AssetOrganisationalSecurityMeasure> OrganisationalSecurityMeasures { get; set; }
        ...
        public void AddApplicableRight(ApplicableRight item)
        {
            // Can only add a new item if it does not already exist.
            if (!ApplicableRights.Any(x => x.ApplicableRight.ApplicableRightId == item.ApplicableRightId))
            {
                ApplicableRights.Add(new AssetApplicableRight()
                {
                    ApplicableRight = item,
                    Asset = this
                });
            }
        }

        public void AddPersonalDataSpecialCategory(PersonalDataSpecialCategory item)
        {
            // Can only add a new item if it does not already exist.
            if (!PersonalDataSpecialCategories.Any(x => x.PersonalDataSpecialCategory.PersonalDataSpecialCategoryId == item.PersonalDataSpecialCategoryId))
            {
                PersonalDataSpecialCategories.Add(new AssetPersonalDataSpecialCategory()
                {
                    PersonalDataSpecialCategory = item,
                    Asset = this
                });
            }
        }

        public void AddSpecialCategoryLawfulProcessingCondition(SpecialCategoryLawfulProcessingCondition item)
        {
            // Can only add a new item if it does not already exist.
            if (!SpecialCategoryLawfulProcessingConditions.Any(x => x.SpecialCategoryLawfulProcessingCondition.SpecialCategoryLawfulProcessingConditionId == item.SpecialCategoryLawfulProcessingConditionId))
            {
                SpecialCategoryLawfulProcessingConditions.Add(new AssetSpecialCategoryLawfulProcessingCondition()
                {
                    SpecialCategoryLawfulProcessingCondition = item,
                    Asset = this
                });
            }
        }

        public void AddPersonalDataSource(PersonalDataSource item)
        {
            // Can only add a new item if it does not already exist.
            if (!PersonalDataSources.Any(x => x.PersonalDataSource.PersonalDataSourceId == item.PersonalDataSourceId))
            {
                PersonalDataSources.Add(new AssetPersonalDataSource()
                {
                    PersonalDataSource = item,
                    Asset = this
                });
            }
        }

        public void AddOrganisationalSecurityMeasure(OrganisationalSecurityMeasure item)
        {
            // Can only add a new item if it does not already exist.
            if (!OrganisationalSecurityMeasures.Any(x => x.OrganisationalSecurityMeasure.OrganisationalSecurityMeasureId == item.OrganisationalSecurityMeasureId))
            {
                OrganisationalSecurityMeasures.Add(new AssetOrganisationalSecurityMeasure()
                {
                    OrganisationalSecurityMeasure = item,
                    Asset = this
                });
            }
        }

        public void AddLawfulProcessingCondition(LawfulProcessingCondition item)
        {
            // Can only add a new item if it does not already exist.
            if (!LawfulProcessingConditions.Any(x => x.LawfulProcessingCondition.LawfulProcessingConditionId == item.LawfulProcessingConditionId))
            {
                LawfulProcessingConditions.Add(new AssetLawfulProcessingCondition()
                {
                    LawfulProcessingCondition = item,
                    Asset = this
                });
            }
        }
